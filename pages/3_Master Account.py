...
if authentication_status:
    ...
    def landing_page():
        ...
        if 'submit_mas' not in state:
            state.submit_mas= False
        if 'response_mas' not in state:
            state.response_mas = []
        st.markdown("<h2 style='text-align: center; padding:0'>Master Account Reconciliation</h2>", unsafe_allow_html=True)
        #st.write('###')
        bank_book, bank_statement, prev_recon, submit= file_upload_form()
        #print(warehouse_reports)
        try:
            if submit:
                state.submit_mas = True
                #print(warehouse_reports)
                #print(submit)
                ...
                with but:
                    st.write("###")
                    with open('temp/master_reconciled.xlsx', 'rb') as my_file:
                        click = st.download_button(label = 'Download in Excel', data = my_file, file_name = 'master_reconciled.xlsx', 
                        mime = 'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet') 
                        #print(click) 
                #st.write(workbook) 

            else:
                if state.submit_mas == True:
                    emp, but, empty = st.columns([2.05,1.2,1.5]) 
                    with but:
                        st.write("###")
                        with open('temp/master_reconciled.xlsx', 'rb') as my_file:
    ...
    def delete_temp():
        os.remove('temp/master_reconciled.xlsx')
